<template>
  <div class="gap-6">
    ...
    <div class="pb-5">
      ...
      <p class="text-[var(--color-icon)] mt-1">Manage your store settings and preferences</p>
    </div>

    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
      <!-- Settings Sidebar -->
      <div class="lg:col-span-1">
        <Card class="sticky top-6 border border-[var(--color-border)] p-2 bg-[var(--color-card)]">
          <nav class="grid grid-cols-1 gap-2">
            <button
              ...
              :class="activeTab === tab.id
                ? 'bg-[var(--color-hover)] text-[var(--primary-color)] border-l-3 border-[var(--primary-color)] font-semibold'
                : 'text-[var(--color-icon)] hover:bg-[var(--color-hover)] hover:text-white hover:border-l-3 hover:border-gray-500/50'"
            >
      ...
      <div class="lg:col-span-3">
        <Card class="p-6 border border-[var(--color-border)] bg-[var(--color-card)]">
          <!-- Store Settings -->
          <div v-if="activeTab === 'store'" class="gap-8">
                  ...
                  <div>
                    ...
                    <div class="relative">
                      <select
                        ...
                        class="appearance-none text-white border border-[var(--color-border)] rounded-lg px-4 py-2.5 pr-10 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 cursor-pointer bg-transparent w-full"
                      >
            ...
            <div class="pt-8 border-t border-[var(--color-border)]">
              <h3 class="text-lg font-semibold text-white mb-6">Store Logo & Branding</h3>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  <label class="block text-sm font-medium text-[var(--color-icon)] mb-3">
                    Store Logo
                  </label>
                  <div
                    class="border-2 border-dashed border-[var(--color-border)] rounded-lg p-6 text-center hover:border-blue-500 transition-colors duration-150 cursor-pointer"
                    @dragover.prevent
                    @drop="handleLogoDrop"
                  ...
                  >
                    ...
                    <div v-else class="mb-4">
                      <svg class="w-16 h-16 mx-auto text-[var(--color-icon)]" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M4 3a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V5a2 2 0 00-2-2H4zm12 12H4l4-8 3 6 2-4 3 6z" clip-rule="evenodd"/>
                      </svg>
                    </div>
                    <div class="mb-4">
                      <p class="text-sm text-[var(--color-icon)] mb-2">
                    ...
                    <div>
                      <button
                        class="px-4 py-2 text-sm border border-[var(--color-border)] rounded-lg text-[var(--color-icon)] hover:bg-[var(--color-hover)] transition-colors duration-150"
                      >
                        Choose Image
                      </button>
                    </div>
                ...
                <div>
                  ...
                  >
                    ...
                    <div v-else class="mb-4">
                      <svg class="w-12 h-12 mx-auto text-[var(--color-icon)]" fill="currentColor" viewBox="0 0 20 20">
          ...
          <div v-else-if="activeTab === 'shipping'" class="gap-8">
            <div>
              <h2 class="text-xl font-semibold text-white mb-6">Shipping Settings</h2>
              <div class="gap-6">
                <div class="flex items-center justify-between mb-4">
                  <div>
                    <h3 class="text-lg font-medium text-white">Shipping Zones</h3>
                    <p class="text-[var(--color-icon)] text-sm">Configure shipping rates for different regions</p>
                  </div>
                  <button
                    @click="addShippingZone"
                    class="flex items-center gap-2 px-4 py-2 bg-[var(--primary-color)] text-white rounded-lg hover:bg-blue-700 transition-colors duration-150"
                  >
                ...
                <div v-if="shippingZones.length === 0" class="text-center py-8 border-2 border-dashed border-[var(--color-border)] rounded-lg">
                  <svg class="w-16 h-16 mx-auto text-[var(--color-icon)] mb-3" fill="currentColor" viewBox="0 0 20 20">
                  ...
                  <p class="text-[var(--color-icon)] mb-4">No shipping zones configured</p>
                  <button
                    @click="addShippingZone"
                    class="px-4 py-2 border border-[var(--color-border)] rounded-lg text-[var(--color-icon)] hover:bg-[var(--color-hover)] transition-colors duration-150"
                  >
                ...
                <div v-else class="gap-4">
                  <div
                    ...
                    :key="zone.id"
                    class="border border-[var(--color-border)] rounded-lg p-4 hover:border-blue-500 transition-colors duration-150"
                  >
                    <div class="flex items-center justify-between mb-3">
                      <h4 class="font-medium text-white">{{ zone.name }}</h4>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      <div>
                        <span class="text-[var(--color-icon)]">Countries:</span>
                        <span class="ml-2 text-white">{{ zone.countries.length }}</span>
                      </div>
                      <div>
                        <span class="text-[var(--color-icon)]">Methods:</span>
                        <span class="ml-2 text-white">{{ zone.methods.length }}</span>
                      </div>
                      <div>
                        <span class="text-[var(--color-icon)]">Status:</span>
                        <span class="ml-2">
                          <span
            ...
            <div class="border-t border-[var(--color-border)] pt-8">
              <h3 class="text-lg font-semibold text-white mb-6">Shipping Options</h3>
              <div class="gap-4">
                <div class="flex items-center justify-between py-3">
                  <div>
                    <label class="font-medium text-white">Free Shipping Threshold</label>
                  ...
                  <div class="w-48">
                    <div class="relative">
                      <span class="absolute left-3 top-2 text-[var(--color-icon)]">
                      ...
                      <input
                        ...
                        min="0"
                        step="0.01"
                        class="w-full pl-8 pr-4 py-2.5 border border-[var(--color-border)] rounded-lg bg-transparent text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      />
                    </div>
                  </div>
                </div>
                ...
                <div class="flex items-center justify-between py-3">
                  <div>
                    <label class="font-medium text-white">Shipping Weight Unit</label>
                    <p class="text-sm text-[var(--color-icon)]">Unit used for shipping weight calculations</p>
                  </div>
                ...
                <div class="flex items-center justify-between py-3">
                  <div>
                    <label class="font-medium text-white">Enable Shipping Calculator</label>
                    <p class="text-sm text-[var(--color-icon)]">Show real-time shipping rates at checkout</p>
                  </div>
          ...
          <div v-else-if="activeTab === 'tax'" class="gap-8">
            <div>
              <h2 class="text-xl font-semibold text-white mb-6">Tax Settings</h2>
              <div class="gap-6">
                <div class="flex items-center justify-between mb-4">
                  <div>
                    <h3 class="text-lg font-medium text-white">Tax Rates</h3>
                    <p class="text-[var(--color-icon)] text-sm">Configure tax rates for different regions</p>
                  </div>
                  <button
                    @click="addTaxRate"
                    class="flex items-center gap-2 px-4 py-2 bg-[var(--primary-color)] text-white rounded-lg hover:bg-blue-700 transition-colors duration-150"
                ...
                <div v-if="taxRates.length === 0" class="text-center py-8 border-2 border-dashed border-[var(--color-border)] rounded-lg">
                  <svg class="w-16 h-16 mx-auto text-[var(--color-icon)] mb-3" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clip-rule="evenodd"/>
                  </svg>
                  <p class="text-[var(--color-icon)] mb-4">No tax rates configured</p>
                  <button
                    @click="addTaxRate"
                    class="px-4 py-2 border border-[var(--color-border)] rounded-lg text-[var(--color-icon)] hover:bg-[var(--color-hover)] transition-colors duration-150"
                  >
                  ...
                  <div class="overflow-x-auto">
                    <table class="w-full">
                      ...
                      <tbody>
                        ...
                        >
                          <td class="py-3 px-4 text-white">{{ rate.country }}</td>
                          <td class="py-3 px-4 text-white">{{ rate.name }}</td>
                          <td class="py-3 px-4 text-white">{{ rate.rate }}%</td>
                          <td class="py-3 px-4">
                            <span
                              class="px-2 py-1 text-xs rounded-full"
                              :class="rate.type === 'inclusive' ? 'bg-green-500/20 text-green-400' : 'bg-blue-500/20 text-blue-400'"
                            >
                              {{ rate.type }}
                            </span>
                          </td>
            ...
            <div class="border-t border-[var(--color-border)] pt-8">
              <h3 class="text-lg font-semibold text-white mb-6">Tax Configuration</h3>
              <div class="gap-4">
                <div class="flex items-center justify-between py-3">
                  <div>
                    <label class="font-medium text-white">Tax Calculation Method</label>
                    <p class="text-sm text-[var(--color-icon)]">How taxes are calculated on orders</p>
                  </div>
                ...
                <div class="flex items-center justify-between py-3">
                  <div>
                    <label class="font-medium text-white">Show Prices With Tax</label>
                    <p class="text-sm text-[var(--color-icon)]">Display prices including tax to customers</p>
                  </div>
                  <label class="relative inline-flex items-center cursor-pointer">
                    <input
                      type="checkbox"
                ...
                <div class="flex items-center justify-between py-3">
                  <div>
                    <label class="font-medium text-white">Tax ID/Number</label>
                    <p class="text-sm text-[var(--color-icon)]">Your business tax identification number</p>
                  </div>
                  <input
                    type="text"
                    v-model="taxSettings.taxId"
                    class="w-64 px-4 py-2.5 border border-[var(--color-border)] rounded-lg bg-transparent text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  />
                </div>
              </div>
            </div>
          ...
          <div v-else-if="activeTab === 'notifications'" class="gap-8">
            <div>
              <h2 class="text-xl font-semibold text-white mb-6">Notification Settings</h2>
              <div class="gap-6">
                <div>
                  ...
                  <div class="gap-4">
                    ...
                    >
                      <div>
                        <label class="font-medium text-white">{{ notification.name }}</label>
                        <p class="text-sm text-[var(--color-icon)]">{{ notification.description }}</p>
                      </div>
                      <label class="relative inline-flex items-center cursor-pointer">
                        <input
                          type="checkbox"
                ...
                <div class="border-t border-[var(--color-border)] pt-8">
                  ...
                  <div class="gap-4">
                    <div class="flex items-center justify-between py-3">
                      <div>
                        <label class="font-medium text-white">Enable Browser Notifications</label>
                        <p class="text-sm text-[var(--color-icon)]">Receive notifications in your browser</p>
                      </div>
                      <button
                ...
                <div class="border-t border-[var(--color-border)] pt-8">
                  ...
                  <div class="gap-4">
                    <div class="flex items-center justify-between py-3">
                      <div>
                        <label class="font-medium text-white">Notification Frequency</label>
                        <p class="text-sm text-[var(--color-icon)]">How often you receive notifications</p>
                      </div>
                      <div class="relative w-48">
                        <select
